...
#define DI_MERGE(x) \
		C_MEM (newdi.x = calloc((ndi.x##_len + outerdi->x##_len), sizeof(outerdi->x[0]))); \
...
static struct {
	...
	/* Josh Rovero <josh.rovero@gmail.com> */
	{"Sony:NEX-5N (PTP mode)",    0x054c, 0x0579, 0},
...
static struct {
	...
	{PTP_OFC_SONY_MPO,		PTP_VENDOR_SONY, GP_MIME_MPO},
	{PTP_OFC_FUJI_RAF,		PTP_VENDOR_FUJI, GP_MIME_RAF},
	{0,				0, NULL}
};
...
#undef XT
	...
	if (params->storageids.len == 0) {
		 ...
		 * this is not supported any sony cameras (mode 3 cameras will return a failure - mode 2 will hang) */
		if (params->deviceinfo.VendorExtensionID != PTP_VENDOR_SONY)
